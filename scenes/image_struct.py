...
class ImageStrcut(Scene):
    def construct(self):
        ...
        gray = VGroup(
            *[Circle(color=color, fill_opacity=1).scale(0.2) for color in table]
        ).arrange()
        transparence = VGroup(
            *[
                Circle(color="#DA1D1D", fill_opacity=0.25 * r).scale(0.2)
                for r in range(5)
            ]
        ).arrange()
        color = VGroup(
            *[Circle(color=color, fill_opacity=1).scale(0.2) for color in color]
        ).arrange()
